...
def plot_pixel_response_relative(
    ...
    plt.tight_layout()
    if not os.path.exists(f"curves_Real/{idx}"):
        os.makedirs(f"curves_Real/{idx}")
    plt.savefig(f"curves_Real/{idx}/position({r}_{c})_ch({ch})_step{step}.png", bbox_inches="tight", dpi=150)
    # plt.show()
    plt.close()
